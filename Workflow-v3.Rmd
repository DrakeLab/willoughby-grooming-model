...
birthrate <- function(x, b0, theta, K){
  ...
  b * pmax(0, 1 - x / K) # birth decreases at high densities
}
...
groomrate_stm <- function(x, scenario){
  groom_stm_scenario <- groom_stm_scenarios %>% dplyr::filter(scenario == scenario) # filter to scenario 
  death_rate <- ifelse(x == 0, 0, # avoids NaNs
                       groom_stm_scenario[1,3])
  death_rate
}
...
groomrate_cst <- function(x, scenario){
  groom_cst_scenario <- groom_cst_scenarios %>% dplyr::filter(scenario == scenario) # filter to scenario 
  death_rate <- ifelse(x == 0, 0, # avoids NaNs
                       exp(-((x - groom_cst_scenario$mu)^2) / (2 * groom_cst_scenario$sigma^2))) # normal distribution
  death_rate
}
...
run_one_parameter_set <- function(task_i){
    ...
    while (time[length(time)] <= max.time) {
      ...
      b <- birthrate(current.size, parms$b0, parms$theta, 300)
...
system.time({
    simulation_test2 <-run_one_parameter_set(1002)
}
)
...
system.time({
  ...
  clusterExport(cl, 
                c( "param_grid", "max.time", "task_grid", "groom_prg_scenarios",
                   "birthrate", "groomrate_prg", "deathrate_dd", "immigrationrate" ))
  
  simulation_pg <- parLapply(cl, 1:nrow(task_grid), run_one_parameter_set)

  on.exit(stopCluster(cl), add = TRUE)
})

saveRDS(simulation_pg, "data/simulation_pg_v3.Rdata")
